...
def counter_plot(fn, display=True):
    ...
    p.extra_y_ranges = {'reward': Range1d(start=min(0, min(y[3])*1.05), end=max(0, max(y[3])*1.05)), 'ratio': Range1d(start=0, end=1.1*max(1, max(max(y[2]), max(y[2]))))}
...
#   p.circle(x, y[0], color='teal', alpha=0.5, size=0.5, legend='score')    # score
    p.triangle(x, y[0], color='red', alpha=0.8, size=2, legend='deaths')     # deaths
    p.inverted_triangle(x, y[1], color='black', alpha=0.8, size=2, legend='timeouts')   # timeouts
    p.circle(x, y[2], color='black', alpha=1, size=5, y_range_name='ratio', legend='ratio')    # ratio
